...
| 透明图(RGBA) | 10 例:5 文生透明 + 5 去背景,全部原生 RGBA,抠图含半透明边缘(§7) |
...
## 7. 透明图测试(RGBA 原生, 10 例)

模型 VAE 为 64 通道 RGBA,**原生输出 alpha 通道**:贴纸 / 透明底商品图 / 抠图可直接出图,无需后处理分割模型。1024×1024 / 30 steps / seed 42,单张 3090 卡(文生透明 GPU0、去背景 GPU1),峰值显存 19.1GB。

- **文生透明图**(5 例):官方推荐提示词格式——用 `This is an RGBA image with transparency. ... The image has alpha channel and the background is transparent.` 包裹主体描述
- **编辑去背景**(5 例):`<image1>` + 指令「只保留主体,整个背景变为透明」,直接输出 RGBA;含一个难例(复杂街景中只抠招牌)
...
10/10 均验证为 RGBA;抠图边缘 alpha 存在 1–254 抗锯齿渐变(半透明边缘列),可直接用于合成。原始 RGBA PNG 与同名提示词 TXT 在 `01_文生透明/`、`02_去背景/`,棋盘格预览如下(原图为透明底):

| 小熊猫贴纸 | 玻璃花瓶 |
|---|---|
| ![](images/05_透明测试/棋盘格预览/a_t1_redpanda_棋盘格.png) | ![](images/05_透明测试/棋盘格预览/a_t2_vase_棋盘格.png) |

| 运动鞋抠图 | 跑车抠图 |
|---|---|
| ![](images/05_透明测试/棋盘格预览/a_t5_sneaker_棋盘格.png) | ![](images/05_透明测试/棋盘格预览/a_e1_car_棋盘格.png) |

| 拉面碗抠图 | 难例:招牌抠图 |
|---|---|
| ![](images/05_透明测试/棋盘格预览/a_e2_ramen_棋盘格.png) | ![](images/05_透明测试/棋盘格预览/a_e5_sign_棋盘格.png) |
...
5. **原生 RGBA 透明**:64 通道 VAE 直接出透明贴纸/抠图,无需后处理分割;文生透明与编辑去背景均可用,边缘带抗锯齿(§7)
...
└── 05_透明测试/         # 10 张 RGBA(5 文生透明 + 5 去背景) + 同名 TXT + 棋盘格预览
data/
